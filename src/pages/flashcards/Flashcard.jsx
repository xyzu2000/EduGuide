...
export default function Flashcard({ term, definition, onEdit, onDelete, creator, isPublic }) {
    ...
    return (
        <div className="relative w-full h-48 perspective" onClick={handleFlip}>
            <div className={`absolute w-full h-full text-white transition-transform duration-1000 transform-style preserve-3d ${flipped ? 'rotate-y-180' : ''}`}>
                ...
                <div className="box-border absolute w-full h-full backface-hidden rotate-y-180 bg-zinc-500 flex justify-center items-center rounded-lg p-4">
                    <div className="text-xl text-wrap overflow-y-auto max-h-full break-words leading-relaxed">
                        {definition}
                    </div>
                </div>
            </div>
